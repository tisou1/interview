...
```
  事件冒泡: 从具体的节点往上逐级传递,直到根节点
  事件捕获: 从根节点开始逐级乡下,直到具体节点

  事件委托就是利用事件冒泡,来将子节点的事件统一冒泡的父节点上进行处理.
```
...
```
  作用域: 作用域是用来定义变量的区域, 他有一套访问变量的机制, 这套规则用来管理引擎如何在当前作用域以及嵌套作用域中根据变量进行查找
         作用域就是一个变量可以使用的范围，主要分为全局作用域和函数作用域和块级作用域
         es6之后有了块级作用域, 像if() {}中用const/let声明的变量只能

  作用域链: 在js使用一个变量时,会首先在当前作用域上找,如果没找到就会往上一层作用域找,直到全局作用域
```

#### 21. 原型和原型链

```
  原型: 每个函数(包括构造函数)都有一个prototype属性, 每个对象都有一个__proto__属性,该属性指向即为原型.

  获取原型的方式还有: Object.getPrototypeOf(obj), Reflect.getPrototypeOf(obj)
